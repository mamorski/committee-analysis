...
class TestRunOneSimulation:
    def test_integration_smoke(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(run_simulations, "start_node", lambda p, i, c: FakeProc(stopped=True))
        monkeypatch.setattr(run_simulations.time, "sleep", lambda *_: None)
        ...
        # Should complete the full lifecycle (configs written, monitor loop exits,
